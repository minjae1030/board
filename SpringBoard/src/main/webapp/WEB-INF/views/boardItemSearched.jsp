...
<table border="1" class="tbl_list">
	...
	<c:forEach var="item" items="${boardItemListSearched.content }" varStatus="status">
		<tr>
...
<br>
<div id="div_page">
	<c:if test="${!boardItemListSearched.first }">
		<a href="${path }/boardItem/${boardId}?page=0">&lt;&lt;&nbsp;맨앞</a>
		<a href="${path }/boardItem/${boardId}?page=${boardItemListSearched.number - 1}">이전</a>
	</c:if>
	...
	<c:if test="${!boardItemListSearched.last }">
		<a href="${path }/boardItem/${boardId}?page=${boardItemListSearched.number + 1}">다음</a>
